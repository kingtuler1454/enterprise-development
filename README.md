...
## Классы
- Car Хранит информацию о характеристиках автомобиля: идентификатор авто, цвет, модель, серийный номер, год выпуска, идентификатор водителя.
- Client Хранит информацию о клиенте: идентификатор клиента, имя и фамилия, номер телефона.
- Driver Хранит информацию о водителе: идентификатор водителя, имя и фамилия, номер телефона, паспортные данные, адрес, идентификатор закрепленного автомобиля.
- Travel Хранит информацию о поездке: идентификатор поездки, пункт отправления, пункт назначения, дата поездки, время в движении.

## Запросы
- ReturnCarDriverInfo Выводит все сведения о конкретном водителе и его автомобиле.
- ReturnPassengetInfo Выводит всех пассажиров, совершивших поездки за заданный период, упорядочивая по ФИО.
- ReturnPassengetTravelsCount Выводит количество поездок каждого пассажира.
- ReturnTopFiveDrives Выводит топ 5 водителей по совершенному количеству поездок.
- ReturnStatisticTime Выводит информацию о количестве поездок, среднем времени и максимальном времени поездки для каждого водителя.
- GetClientsMaxTrips Выводит информацию о пассажирах, совершивших максимальное число поездок за указанный период.

## Задание
14. «Таксопарк»
Исходные данные:
Таксопарк хранит информацию:
- водителях (ФИО, паспорт, адрес, телефон), 
- автомобилях (гос.номер, модель, цвет, …), 
- пользователях сервиса (телефон, ФИО)  
- поездках (пункты отправления и назначения, дата поездки, время движения, стоимость, назначенное такси). Каждый автомобиль закреплен за одним водителем.
Запросы:
